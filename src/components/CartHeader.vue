<script setup lang="ts">
import { inject } from "vue";

interface CartActions {
  closeDrawer: () => void;
}
const { closeDrawer } = inject<CartActions>('cartActions')

</script>
<template>

  <h2 class="text-2xl font-bold mb-10 flex items-center gap-5">
    <svg @click="closeDrawer" class="rotate-180 hover:-translate-x-1 opacity-30 hover:opacity-100 transition cursor-pointer" width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M1 7H14.7143" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      <path d="M8.71436 1L14.7144 7L8.71436 13" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>
    Корзина
  </h2>

</template>